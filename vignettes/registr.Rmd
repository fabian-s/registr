...
`register_fpca()` is the main function for the `registr` package. Use the `family` argument to this function to specify what type of exponential family data you would like to align. The package supports `family = "gaussian"` for registering continuous data, `family = "binomial"` for registering binary data, `family = "gamma"` for strictly positive data with a Gamma structure and `family = "poisson"` for nonnegative data with a Poisson structure.
...
registr_bin = register_fpca(Y = registration_data, family = "binomial", Kt = 8, Kh = 4, npc = 1)
```

The argument `Y` specifies the input dataset; this code uses the simulated `registration_data`. `Kt` and `Kh` specify number of B-spline basis functions for the subject-specific means and warping functions, respectively, and `npc` indicates the number of functional principal components to use.
...
In contrast to the variational EM based GFPCA approaches of @wrobel_2019
(`fpca_gauss()` and `bfpca()`) the number of functional principal components
can be adaptively chosen by `gfpca_twoStep()` based on the share of explained
variance. See `?gfpca_twoStep()`.

Our implementation is based on the codebase accompanying their paper which can
be found at [github.com/jeff-goldsmith/gfpca](https://www.github.com/jeff-goldsmith/gfpca).
...
```{r register_parametric_plots, echo = FALSE, fig.show='hold'}
ggplot(registr_gauss$Y, aes(x = tstar, y = t_hat, group = id)) +
	geom_line() + 
	labs(title = "warping = nonparametric")

ggplot(registr_parametric$Y, aes(x = tstar, y = t_hat, group = id)) +
	geom_line() + 
...
ggplot(registr_par_priors$Y, aes(x = tstar, y = t_hat, group = id)) +
	geom_line() + 
	labs(title = "sd for all priors = 0.1")

ggplot(registr_par_priors2$Y, aes(x = tstar, y = t_hat, group = id)) +
	geom_line() + 
...
tibble(mu = registr_non_periodic$fpca_obj$mu) %>%
...
tibble(psi1 = registr_non_periodic$fpca_obj$efunctions[,1]) %>%
	mutate(time = row_number()) %>%
...
tibble(mu = registr_periodic$fpca_obj$mu) %>%
	mutate(time = row_number()) %>%
	ggplot(aes(x = time, y = mu)) + 
	theme_bw() + 
	geom_line() + 
...
tibble(psi1 = registr_periodic$fpca_obj$efunctions[,1]) %>%
	mutate(time = row_number()) %>%
	ggplot(aes(x = time, y = psi1)) + 
	theme_bw() + 
	geom_line() + 
